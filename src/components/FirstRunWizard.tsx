...
export function FirstRunWizard({ hasProject, onAddProject, onConfigure, onClose }: Props) {
  ...
  return <div className="first-run-backdrop"><section className="first-run" role="dialog" aria-modal="true" aria-labelledby="first-run-title">
    ...
    {step === 0 ? <main><div className="first-run-hero"><Laptop size={28} /><h1 id="first-run-title">{t("Tu código permanece bajo tu control", "Your code stays under your control")}</h1><p>{t("Trabaja con modelos locales o conecta tu propio proveedor. Nova explica los errores y nunca ejecuta cambios fuera del proyecto.", "Use local models or connect your own provider. Nova explains errors and never changes files outside the project.")}</p></div><div className="first-run-points"><span><ShieldCheck size={16} /><b>{t("Claves protegidas por el sistema", "Keys protected by the system")}</b></span><span><FolderPlus size={16} /><b>{t("Acceso limitado al proyecto", "Project-scoped access")}</b></span><span><Check size={16} /><b>{t("Revisión y recuperación de cambios", "Change review and recovery")}</b></span></div></main> : <main><div className="first-run-hero"><KeyRound size={28} /><h1 id="first-run-title">{t("Prepara tu espacio de trabajo", "Prepare your workspace")}</h1><p>{t("Añade una carpeta y configura un modelo. Puedes cambiar ambas cosas después.", "Add a folder and configure a model. You can change both later.")}</p></div><div className="first-run-actions"><button className={hasProject ? "is-complete" : ""} onClick={onAddProject}><FolderPlus size={18} /><span><strong>{hasProject ? t("Proyecto preparado", "Project ready") : t("Añadir proyecto", "Add project")}</strong><small>{t("Selecciona una carpeta de este equipo", "Select a folder on this computer")}</small></span>{hasProject && <Check size={16} />}</button><button onClick={() => finish(onConfigure)}><KeyRound size={18} /><span><strong>{t("Configurar modelo", "Configure model")}</strong><small>{t("Ollama, LM Studio o una API", "Ollama, LM Studio, or an API")}</small></span><ArrowRight size={16} /></button></div></main>}

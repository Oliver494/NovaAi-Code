...
export function ChatModelPicker({ projectPath, settings, disabled, onChange, onConfigure }: Props) {
  ...
  const effortSupported = supportsReasoningEffort(selectedConfig.provider, selectedConfig.model);
  const canLoad = !providerMeta[selectedConfig.provider].requiresKey || selectedConfig.apiKeyConfigured;
  ...
  useEffect(() => {
    const next = activeProviderConfig(settings);
  ...
  async function loadModels(configId = selected) {
  ...
  async function chooseProvider(config: ProviderConfig) {
    ...
    try {
      ...
      if (!providerMeta[savedConfig.provider].requiresKey || savedConfig.apiKeyConfigured) {
        setLoading(true);
  ...
  async function chooseModel(model: string) {
    ...
    try {
      const next: AiSettings = {
        ...
        providers: settings.providers.map((item) => item.configId === selected ? { ...item, model } : item),
      };
  ...
  async function chooseEffort(reasoningEffort: ReasoningEffort) {
    ...
    try {
      const next: AiSettings = {
        activeProvider: selectedConfig.provider,
        activeConfigId: selectedConfig.configId,
        providers: settings.providers.map((item) => item.configId === selected ? { ...item, reasoningEffort } : item),
      };
      onChange(await ai.saveSettings(projectPath, next));
    } catch (cause) { setError(asDiagnostic(cause).explanation); }
    finally { setSaving(false); }
      ...
      <div className="model-provider-grid">
        {orderedConfigs.map((config) => {
          return <button type="button" key={config.configId} className={selected === config.configId ? "is-active" : ""} onClick={() => void chooseProvider(config)} disabled={saving}>
            ...
            <em>{providerMeta[config.provider].requiresKey && !config.apiKeyConfigured ? <KeyRound size={11} /> : config.model ? <Check size={11} /> : null}</em>
          </button>;
        })}
      </div>

...
const system = `Eres Vareliox, un asistente de programación integrado en Vareliox Code.
...
const chatSystem = "Eres Vareliox Chat, un asistente conversacional. Responde preguntas, explica y genera ejemplos, pero no tienes acceso al proyecto ni puedes crear, editar, mover, eliminar o afirmar que modificaste archivos. Si el usuario pide cambios, entrega el código en el chat e indica brevemente que puede cambiar a Vareliox Code para aplicarlos.";
...
const cases = [
  ...
  { name: "Vareliox Chat no activa el agente", mode: "chat", messages: [{ role: "user", content: "créame un archivo login.html" }], expectAction: false },
];

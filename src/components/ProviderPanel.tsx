  ...
  async function persist(nextDraft = draft, savePendingKey = true) {
    ...
    try {
      ...
      const next: AiSettings = {
        activeProvider: nextDraft.provider,
        activeConfigId: nextDraft.configId,
    ...
    <div className="provider-panel">
      ...
      <div className="provider-panel__body">
        ...
        <section className="provider-form">
          ...
          {draft.model && (supportsReasoningEffort(draft.provider, draft.model) ? <label>{t("Esfuerzo", "Effort")}<span className="field-hint">{t("Controla cuánto razona el modelo antes de responder.", "Controls how much the model reasons before responding.")}</span><div className="provider-effort" role="group" aria-label={t("Esfuerzo de razonamiento", "Reasoning effort")}>{(["low", "medium", "high"] as ReasoningEffort[]).map((effort) => <button type="button" key={effort} className={(draft.reasoningEffort ?? "medium") === effort ? "is-active" : ""} onClick={() => void selectEffort(effort)} disabled={saving}>{effort === "low" ? t("Bajo", "Low") : effort === "medium" ? t("Medio", "Medium") : t("Alto", "High")}</button>)}</div></label> : <div className="provider-effort-unavailable"><strong>{t("Esfuerzo predeterminado", "Default effort")}</strong><span>{t("Este modelo no permite cambiarlo.", "This model does not allow changing it.")}</span></div>)}
          ...
          <footer><span>{saving ? t("Guardando cambios…", "Saving changes…") : t("Configuración segura del proyecto", "Secure project settings")}</span>{draft.provider === "custom" && <button className="danger-text-button" disabled={saving || testing} onClick={() => void deleteCustomProvider()}><Trash2 size={14} />{t("Eliminar proveedor", "Delete provider")}</button>}<button className="secondary-button" disabled={saving || testing || (draft.provider === "custom" && !draft.displayName.trim())} onClick={() => void persist()}>{saving ? <LoaderCircle className="spin" size={14} /> : <Check size={14} />}{t("Guardar", "Save")}</button><button className="primary-button" disabled={saving || testing || (draft.provider === "custom" && !draft.displayName.trim())} onClick={() => void test()}>{testing ? <LoaderCircle className="spin" size={14} /> : <RefreshCw size={14} />}{testing ? t("Probando conexión…", "Testing connection…") : t("Probar conexión", "Test connection")}</button></footer>
        </section>
      </div>
    </div>
    {catalogOpen && (draft.provider === "ollama" || draft.provider === "lm_studio") && <LocalModelCatalog provider={draft.provider} config={draft} onClose={() => setCatalogOpen(false)} onInstalled={() => void loadModels()} />}
  </div>;
}
...
function ProviderGroup({ title, icon, items, selected, onSelect }: { title: string; icon: React.ReactNode; items: ProviderConfig[]; selected: string; onSelect: (id: string) => void }) {
  const { t } = usePreferences();
  return <div className="provider-group"><span>{icon}{title}</span>{items.map((item) => <button key={item.configId} className={selected === item.configId ? "is-active" : ""} onClick={() => onSelect(item.configId)}><ProviderLogo provider={item.provider} customLogo={item.logoDataUrl} label={providerDisplayName(item)} size="medium" /><span><strong>{providerDisplayName(item)}</strong><small>{item.model || (providerMeta[item.provider].type === "local" ? t("Local", "Local") : t("Sin modelo", "No model"))}</small></span><em className={item.apiKeyConfigured || providerMeta[item.provider].type === "local" ? "is-ready" : ""}>{item.apiKeyConfigured ? <KeyRound size={12} /> : <i />}</em></button>)}</div>;
}

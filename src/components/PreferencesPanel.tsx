...
export function PreferencesPanel({ onClose, onOpenProviders, projectPath, settings, onFilesRestored }: Props) {
  ...
  const themes: { id: ThemePreference; label: string; description: string; icon: typeof Monitor }[] = [
    { id: "system", label: t("Sistema", "System"), description: t("Sigue el sistema", "Follows the system"), icon: Monitor },
  ...
  const languages: { id: LanguagePreference; name: string; nativeName: string; detail: string }[] = [
    { id: "auto", name: t("Automático", "Automatic"), nativeName: t("Idioma del sistema", "System language"), detail: languageNames[resolvedLanguage][0] },
  ...
  return <div className="preferences-overlay" role="dialog" aria-modal="true" aria-label={t("Configuración", "Settings")}>
    <section className="preferences-panel">
      <header><div><strong>{t("Configuración", "Settings")}</strong><span>{t("Personaliza Vareliox Code", "Customize Vareliox Code")}</span></div><button className="icon-button" onClick={onClose} aria-label={t("Cerrar", "Close")}><X size={18} /></button></header>
      <div className="preferences-layout">
        ...
        <main className="preferences-content">
          <div className="preferences-page-title"><strong>{activeLabel}</strong><span>{section === "appearance" ? t("Elige cómo se muestra Vareliox Code", "Choose how Vareliox Code looks") : section === "language" ? t("Automático usa el idioma del sistema", "Automatic uses the system language") : section === "permissions" ? t("Decide a qué puede acceder Vareliox y qué acciones puede realizar", "Choose what Vareliox can access and which actions it can perform") : section === "terminal" ? t("Controla qué comandos puede ejecutar el agente", "Control which commands the agent can run") : section === "doctor" ? t("Comprueba el proyecto y la conexión activa", "Check the project and active connection") : section === "hardware" ? t("Comprueba qué modelos locales caben en tu equipo", "Check which local models fit your hardware") : section === "git" ? t("Revisa cambios sin modificar el repositorio", "Inspect changes without modifying the repository") : section === "recovery" ? t("Restaura cambios recientes del agente", "Restore recent agent changes") : t("Controla cómo recibes nuevas versiones", "Control how you receive new versions")}</span></div>

...
export function PermissionsPanel() {
  ...
  function chooseFullAccess() {
    ...
    const accepted = window.confirm(t(
      "El acceso completo permite a Vareliox Code trabajar en cualquier carpeta del equipo cuando se lo pidas. Los cambios fuera del proyecto siempre requerirán tu aprobación. ¿Quieres activarlo?",
      "Full access lets Vareliox Code work in any computer folder when you ask. Changes outside the project will always require your approval. Enable it?",
    ));
  ...
  return <section className="permissions-settings">
    ...
    {permissions.computerAccess === "full" && <div className="permission-warning"><AlertTriangle size={17} /><span><strong>{t("Permiso sensible activado", "Sensitive permission enabled")}</strong><small>{t("Vareliox no examina todo el disco automáticamente. Solo usa rutas relacionadas con tu petición y nunca puede modificar una carpeta externa sin mostrarte la operación.", "Vareliox does not scan the whole disk automatically. It only uses paths related to your request and can never change an external folder without showing you the operation.")}</small></span></div>}

    <h3>{t("Capacidades", "Capabilities")}</h3>
    <div className="permission-toggle-list">
      <PermissionToggle icon={ShieldCheck} title={t("Crear y editar archivos", "Create and edit files")} description={t("Permite a Vareliox Code aplicar cambios en ubicaciones autorizadas.", "Allow Vareliox Code to apply changes in authorized locations.")} checked={permissions.allowFileChanges} onChange={(value) => update("allowFileChanges", value)} />
    ...
    <p className="permissions-footnote"><ShieldCheck size={14} />{t("Configuración inicial segura: Vareliox solo accede al proyecto abierto.", "Safe default: Vareliox only accesses the open project.")}</p>
  </section>;
}
